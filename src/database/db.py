...
class Database:
    def __init__(self):
        if not os.path.exists("src/database/cookpaw.db"):
            self.conn = sqlite3.connect("src/database/cookpaw.db")
            c = self.conn.cursor()
            c.execute("""
            ...
            VALUES (
                "Indonesian Cuisine: A Delicious and Diverse Culinary Experience",
            ...
            INSERT INTO articles (title, content, publish_date)
            VALUES (
                "Exploring the Delicious World of Indonesian Street Food",
            ...
            """)
            c.execute("""
            INSERT INTO recipes (title, utensils, ingredients, steps, last_modified)
            VALUES (
                "Spicy Shrimp Tacos",
                "grill pan, tongs, mixing bowls, cutting board, knife",
                "1 lb large shrimp, peeled and deveined; 8-10 small tortillas; 1 avocado, diced; 1/2 red onion, sliced; 1 jalapeno, sliced; 1/4 cup chopped cilantro; 2 tbsp olive oil; 2 tbsp taco seasoning",
            ...
            VALUES (
                ...
                "Large skillet, wooden spoon, cutting board, chef’s knife, mixing bowl",
                "1 lb boneless chicken thighs, cut into small pieces; 1 cup fresh basil leaves; 1 red bell pepper, sliced; 2 garlic cloves, minced; 2 Thai chilies, sliced; 2 tbsp soy sauce; 1 tbsp oyster sauce; 1 tbsp fish sauce; 1 tbsp brown sugar; 1 tbsp vegetable oil",
                "1. Heat vegetable oil in a large skillet over medium-high heat. Add garlic and chilies and cook until fragrant, about 1 minute.\n2. Add chicken and stir-fry until cooked through, about 5-7 minutes.\n3. Add red bell pepper, soy sauce, oyster sauce, fish sauce, and brown sugar. Stir-fry for another 2-3 minutes.\n4. Add fresh basil leaves and stir-fry until wilted, about 1 minute.\n5. Serve hot with steamed rice.",
                "2023-04-16"
                );
            """)
            c.execute("""
            ...
            VALUES (
                "images_article/indonesia_cuisine.jpg"
                );
            """)
            c.execute("""
            INSERT INTO photos (path)
            VALUES (
                "images_article/indonesia_streetfood.jpg"
                );
            """)
            c.execute("""
            INSERT INTO photos (path)
            ...
            INSERT INTO photos (path)
            VALUES (
                "images_article/french_cooking.jpg"
                );
            """)
            c.execute("""
            INSERT INTO photos (path)
            VALUES (
                "images_recipe/spicy_shrimp_taco.jpg"
                );
            """)
            c.execute("""
            INSERT INTO photos (path)
            VALUES (
                "images_recipe/spicy_thai_basil.jpg"
                );
            """)
            c.execute("""
            INSERT INTO photos (path)
            VALUES (
                "images_recipe/creamy_mushroom_risotto.jpg"
                );
            """)
            c.execute("""
            INSERT INTO article_photos
            VALUES (
                1, 1
                );
            """)

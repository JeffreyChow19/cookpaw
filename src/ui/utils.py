...
def getFont(font_weight, font_size):
    ...
    font_path = f"img/fonts/poppins/Poppins-{font_weight}.ttf"
...
def save_image_to_img(source_image_path, content_type="recipe"):
    """Copy and save an external image to img folder; supported content_type are recipe and notes"""
    if content_type != "recipe" and content_type != "notes":
        content_type = "recipe"
        
    # copy source image to img
    destination_path = f'img/images/images_{content_type}/' + os.path.basename(source_image_path)
    shutil.copy(source_image_path, destination_path)

    # handle complex icc profiles in img folder
